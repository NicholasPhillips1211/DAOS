...
class AutomationService:
    ...
    async def generate_plan(self, db: Session, workspace_id: int, objective: str) -> AutomationPlan:
        ...
        plan_payload = self._attach_ai_trace(plan_payload, objective=objective, signals=signals, provider=provider)
    ...
    def _build_fallback_plan(self, objective: str, signals: WorkspaceSignals) -> dict[str, Any]:
        ...
        return {
            ...
            "confidence_score": self._estimate_confidence(signals, provider="heuristic"),
            "triggers": triggers,
            "actions": actions,
    ...
    def _normalize_llm_payload(
        ...
        normalized = {
            ...
            "confidence_score": self._normalize_confidence(
                ...
                self._estimate_confidence(signals, provider="lm-studio"),
            ),
    ...
    def _attach_ai_trace(
        ...
        *,
        ...
        """Ensure every AI plan has confidence and grounding metadata for explainability."""
        ...
        trace = {
            ...
            "objective": objective,
            "grounding": {
                "signal_snapshot": asdict(signals),
                "evidence": self._build_grounding_evidence(signals),
            },
        }

        payload["confidence_score"] = self._normalize_confidence(
            payload.get("confidence_score"),
            self._estimate_confidence(signals, provider=provider),
        )
        payload["trace"] = trace
        return payload

    def _build_grounding_evidence(self, signals: WorkspaceSignals) -> list[str]:
        evidence: list[str] = [f"dataset_count={signals.dataset_count}", f"pipeline_count={signals.pipeline_count}"]
        if signals.dashboard_count == 0:
            evidence.append("No dashboards found; recommend operational dashboard baseline")
        if signals.model_count == 0 and signals.dataset_count > 0:
            evidence.append("Datasets exist without trained models; recommend baseline model training")
        if signals.comment_count == 0 or signals.share_count == 0:
            evidence.append("Collaboration signal is weak; recommend comments/sharing activation")
        return evidence

    def _estimate_confidence(self, signals: WorkspaceSignals, provider: str) -> float:
        base = 0.62 if provider == "heuristic" else 0.74
        maturity_bonus = min(0.18, (signals.dataset_count * 0.03) + (signals.pipeline_count * 0.02))
        data_penalty = 0.08 if signals.dataset_count == 0 else 0.0
        return round(max(0.35, min(0.98, base + maturity_bonus - data_penalty)), 2)
