...
| TD-005 | Information Collection | Upload processing remains request-bound | Upload acceptance now queues worker-side profiling through `WorkItem`, but profiler internals still read full CSV files into memory. | Request blocking is addressed; large-file memory pressure can still occur inside the worker. | P0 | Replace full-file CSV profiling with streaming or DuckDB-backed profiling. | Addressed |
...
| TD-007 | Information Collection | Worker handoff missing | Ingestion jobs now link to `WorkItem` records and worker execution handles queued profiling with stale-lock recovery. | Durable worker handoff exists; retry and progress semantics should be deepened for production scale. | P0 | Add resumable profiling checkpoints, worker heartbeats, and richer retry policy controls. | Addressed |
