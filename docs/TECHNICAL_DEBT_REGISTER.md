...
| TD-009 | Information Governance | SQL metadata emission is shallow | SQL execution now emits usage metadata, but not query lineage or persisted execution records. | Metadata can see dataset usage, but cannot yet answer which SQL assets power which downstream outputs. | P0 | Add query execution records and lineage metadata from query execution. | In progress |
| TD-010 | Information Operationalization | Dashboard dependencies missing | Dashboard creation now emits usage metadata, but dependencies and KPI ownership are not first-class. | Dashboards remain weak operational assets without dependencies, ownership, and impact analysis. | P0 | Add dashboard dependency records, KPI ownership, and impact analysis. | In progress |
| TD-011 | Information Intelligence | AI context builder missing | Dataset profiles and automation plans now create AI context records. | AI has grounding snapshots, but lacks a reusable context builder from metadata, profiles, lineage, query history, dashboards, governance, and workspace state. | P0 | Build the AI context builder and standard grounded response contract. | In progress |
